...
export default function DashboardPage({ stats, recentActivity }) {
  const cards = [
    {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        {cards.map((card, index) => (
          <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              <div className={`p-3 lg:p-4 rounded-lg ${card.bgColor}`}>
                <card.icon className={`${card.textColor} text-2xl lg:text-3xl`} />
              </div>
            ...
            <div>
              <h3 className="text-xl lg:text-2xl font-bold text-gray-800 mb-1">{card.value}</h3>
            ...
            {recentActivity?.slice(0, 5).map((activity, index) => (
              <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-800 truncate">{activity.name}</p>
                  <p className="text-sm text-gray-600 truncate">{activity.email} • {activity.date}</p>
                </div>
